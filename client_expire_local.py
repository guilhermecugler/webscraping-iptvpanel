...
#   abre o navegador e manda mensagem para os clientes que
...
for clientId in clientsJson['data']:
    ...
    phone = soupClient.find("input", {'name': "phone_number"}).attrs['value']
    expiredate = clientId[2].date()
    delta = expiredate - date.today()
    strexpiredate = clientId[2].strftime("%d/%m/%Y às %H:%M")

    if(delta.days <= 2 and phone!=""):
        now = datetime.now()
        py.sendwhatmsg(f'+55{phone}', f'Olá, seus canais vencem dia *{strexpiredate}*', now.hour, now.minute*1+1, tab_close=True)
